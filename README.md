...
### Keep
- 예쁜 디자인
- 백엔드와 프론트엔드, DB의 완전한 분리
- 기획이 바뀔 때(카드/도감 → 별/은하 컨셉 전환 등) 화면과 API 이름·구조를 처음부터 다시 맞춘 점
...
### Try
- 초기 기획·DB 설계 단계에서 "나중에 뒤집힐 수 있는" 결정(카드 vs 별, 팀 삭제 정책 등)을 더 미리 점검하기
- 배포 전에 실제 사용자 시나리오(팀 생성→평가→탈퇴 등 전체 플로우) 기준 통합 테스트를 더 강화하기
