# 매드노바 도감 백엔드 개발 계획 (BACKEND_DEVELOPMENT_PLAN)
...
|---|---|
| 문서 작성일 | 2026-07-03 |
| 대상 프로젝트 | 매드노바 도감 (몰입캠프 참가자 상호 평가 및 팀 빌딩 플랫폼) |
...
KAIST 몰입캠프 참가자들이 프로젝트를 함께 진행한 팀원을 대상으로 상호 평가를 진행하고, 그 결과를 게임 카드(매드노바 카드) 형태로 시각화하여 서로의 강점과 특징을 재미있게 공유하는 서비스입니다. 단순한 평가 도구가 아니라 **게임화(gamification)** 와 **AI 기반 분석**을 결합해 참가자 간 네트워킹과 팀 빌딩 경험을 향상시키는 것이 목적입니다.
...
3. **상호 평가** — 프로젝트 종료 후 같은 팀원끼리만 6개 항목(체력/공격력/방어력/마력/민첩성/협동력, 각 1~10점) 평가, 평가는 제출 후 수정 불가
...
- 초기 능력치 6종: `initial_attack`, `initial_defense`, `initial_agility`, `initial_teamwork`, `initial_mana`, `initial_health`
...
- 6개 항목 원점수: `attack`, `defense`, `agility`, `teamwork`, `mana`, `health` (각 1~10)
