...
import com.madmon.main.chat.dto.ChatCardBrief;
...
// AI 카드 분석(§8). Card 도메인이 제공하는 요약 데이터(능력치, 대표 칭호, 자기소개)를
// 그대로 프롬프트 컨텍스트로 사용하므로 card 패키지 완성 이후에 구현한다(Phase 9 선행).
@Service
...
public class ChatService {
    ...
    private final ChatCardRepository chatCardRepository;
    ...
    public ChatSessionResponse createSession(Long userId, CreateSessionRequest request) {
        ...
        for (User target : targets) {
            chatCardRepository.save(ChatCard.create(session, target));
        }
    ...
    public ChatSessionDetailResponse getSessionDetail(Long userId, Long sessionId) {
        ...
        return new ChatSessionDetailResponse(
                ...
                targets.stream().map(this::toCardBrief).toList(),
    ...
    private String buildIndividualSystemPrompt(Long viewerId, User target) {
        return """
                당신은 몰입캠프 참가자 카드 도감의 AI 분석 도우미입니다.
                아래는 개별 카드 질문의 분석 대상 참가자 정보입니다.
                %s
                이 정보를 바탕으로 사용자의 질문에 한국어로 답하세요. 정보에 없는 내용은 추측하지 말고 모른다고 답하세요.
                """.formatted(describeCard(cardService.getCardDetail(viewerId, target.getId())));
    }

    private String buildGroupSystemPrompt(Long viewerId, List<User> targets) {
        String cardsDescription = targets.stream()
                .map(target -> describeCard(cardService.getCardDetail(viewerId, target.getId())))
        ...
        return """
                ...
                """.formatted(targets.size(), cardsDescription);
    }

    private String describeCard(CardDetailResponse card) {
        StringBuilder description = new StringBuilder();
        description.append("- 이름: ").append(card.name()).append('\n');

        if (card.representativeTitles() != null && !card.representativeTitles().isEmpty()) {
        ...
        if (card.biography() != null && !card.biography().isBlank()) {
            description.append("  자기소개: ").append(card.biography()).append('\n');
        }
        return description.toString();
    }

    // Chat 기능 전체가 카드 상세 잠금 여부에 연동된다(기능명세서 4.6.4).
    // 카드 도감의 판정 로직(Phase 9)을 그대로 재사용한다.
    private void requireUnlocked(Long viewerId) {
        if (!cardService.evaluateLockStatus(viewerId).unlocked()) {
    ...
    private List<User> targetsOf(ChatSession session) {
        return chatCardRepository.findAllBySessionId(session.getId()).stream()
                .map(ChatCard::getTargetUser)
                .toList();
    }
    ...
    private ChatCardBrief toCardBrief(User user) {
        return new ChatCardBrief(user.getId(), user.getName(), user.getProfileImageUrl());
    }
